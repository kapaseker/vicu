...
- FFmpeg functionality comes from `app/libs/ffmpeg-kit-next-api.aar` (API-only, `jni/` stripped) plus the checked-in native libraries under `app/src/main/jniLibs/`; the API AAR and `.so` files are split artifacts of the full `ffmpeg-kit-next` 9.0.0 AAR to stay under GitHub's 100 MB file limit. For SDK usage, refer to the official Android docs: https://github.com/arthenica/ffmpeg-kit-next/tree/main/android.
